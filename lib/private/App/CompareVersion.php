...
class CompareVersion {
	...
	public function isCompatible(string $actual, string $required,
		...
		if (!preg_match(self::REGEX_MAJOR_MINOR_PATCH, $actual)) {
			throw new InvalidArgumentException('server version is invalid. use major.minor.patch string');
		}
		...
		if (preg_match(self::REGEX_MAJOR, $required) === 1) {
			...
			return version_compare($actual, $required, $comparator);
		}
	}
